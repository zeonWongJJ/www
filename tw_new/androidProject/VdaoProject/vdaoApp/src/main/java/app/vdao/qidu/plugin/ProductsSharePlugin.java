...
public class ProductsSharePlugin extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray array,final CallbackContext callbackContext) throws JSONException {
        ...
        }else if(action.equals(PluginAction.ACTION_SHOW_INPUT_DIALOG_FOR_PRODUCTS_PRICE)){
    ...
    private void showInputDialogForProductPrice(final CallbackContext callbackContext) {//条件选择器初始化，自定义布局
        InputPriceDialog inputPriceDialog=new InputPriceDialog(mainCordovaActivity);
        inputPriceDialog
                .builder()
                .setTitle("请输入单价")
                .setCancelable(true)
                .setCanceledOnTouchOutside(true)
                .setOnEditListener(new InputPriceDialog.OnEditListener() {
                    @Override
                    public void onEditInputResult(String content) {

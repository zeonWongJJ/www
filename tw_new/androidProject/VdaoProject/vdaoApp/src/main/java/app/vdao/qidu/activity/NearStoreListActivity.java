...
public class NearStoreListActivity extends BaseMarkActivity implements LocationSource, AMapLocationListener, AMap.OnCameraChangeListener,AMap.OnMarkerClickListener {
    ...
    private void filterData(String filterStr) {
        if(listStores.size()==0){
            ...
            String json= (String) SharedPreferencesUtils.getInstance().getData("storeListJson","");
    ...
    private void sortList(final List<Store> storeList){
        ...
        Disposable disposable = (Disposable)observable.subscribeOn(Schedulers.io()).unsubscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribeWith(new DisposableObserver<List<Store>>() {
            ...
            public void onNext(List<Store> list) {
                ...
                SharedPreferencesUtils.getInstance().saveData("storeListJson",str);

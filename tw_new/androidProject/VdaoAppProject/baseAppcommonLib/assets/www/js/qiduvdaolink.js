    ...
    //打开新页面
    //"showTitleType":0}//目前0,是默认打开包括标题栏布局显示，1 标题栏布局不显示，2 标题栏布局显示，但根布局是frameLayout --用于客服页面打开
        //{"url":"http:","showTitleType":0}
    ...
    //标题栏配置-是否可见{"isShowTitle":0} 0显示 1不显示
    var titleBarLayoutIsVisible=function (str){
        	    var onDeviceReady = function() {
                     cordova.exec(null, null, "MainCordovaActivityPlugin", "titleBarLayoutIsVisible", [str]);
                };
        	    document.addEventListener("deviceready", onDeviceReady, true);
        }
    ...
    /*{"user_id":1}*/
	var loginOut=function (obj,callbackSuccess){
    	    var onDeviceReady = function() {
                 cordova.exec(callbackSuccess, null, "MainCordovaActivityPlugin", "loginOut", [obj]);
            };
    	    document.addEventListener("deviceready", onDeviceReady, true);
    	}
    ...
    var isArrowWindowFinish=function(str){
            var onDeviceReady = function() {
                      cordova.exec(null, null, "MainCordovaActivityPlugin", "isArrowWindowFinish", [str]);
             };
             document.addEventListener("deviceready", onDeviceReady, true);
     }
    //获取设备id
    var initSystemParams=function (callbackSuccess,str){
        var onDeviceReady = function() {
             cordova.exec(callbackSuccess, null, "MainCordovaActivityPlugin", "initSystemParams", [str]);
        };
        document.addEventListener("deviceready", onDeviceReady, true);
    }
    //分享

...
import com.app.base.treeview.model.ForTreeItem;
...
public class RightTreeLayoutManager implements TreeLayoutManager {
    ...
    public void onTreeLayout(final TreeView treeView) {
        ...
        final TreeModel<StructureBean> mTreeModel = treeView.getTreeModel();
        if (mTreeModel != null) {
            ...
            mTreeModel.addForTreeItem(new ForTreeItem<NodeModel<StructureBean>>() {
                @Override
                public void next(int msg, NodeModel<StructureBean> next) {
    ...
    private void doNext(int msg, NodeModel<StructureBean> next, TreeView treeView) {
    ...
    public void correctLayout(TreeView treeView, NodeView next) {
        ...
        if (next.getParent() != null && count >= 2) {
            NodeModel<StructureBean> tn = next.getTreeNode().getChildNodes().get(0);
            NodeModel<StructureBean> bn = next.getTreeNode().getChildNodes().get(count - 1);
            ...
            //上移动
            ArrayList<NodeModel<StructureBean>> allLowNodes = mTree.getAllLowNodes(bn);
            ArrayList<NodeModel<StructureBean>> allPreNodes = mTree.getAllPreNodes(tn);

            for (NodeModel<StructureBean> low : allLowNodes) {
                NodeView view = (NodeView) treeView.findNodeViewFromNodeModel(low);
                moveNodeLayout(treeView, view, bnDr);
            }

            for (NodeModel<StructureBean> pre : allPreNodes) {
    ...
    private void standardLayout(TreeView treeView, NodeView rootView) {
        NodeModel<StructureBean> treeNode = rootView.getTreeNode();
        if (treeNode != null) {
            //所有的子节点
            LinkedList<NodeModel<StructureBean>> childNodes = treeNode.getChildNodes();

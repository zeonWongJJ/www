			...
			$.ajax({
				...
				success: function(res) {
					...
					if (res.code == 200) {
						...
						$('.shade').hide();
					    $('.cancelBomb').hide();
					    _this.parent('.control').html('<a href="office_detail-'+office_id+'" class="bookAgain" orid="'+appointment_id+'">再次预约</a>');
					    _this.remove();
			...
			$.ajax({
				...
				success: function(res) {
					...
					if (res.code == 200) {
						...
						$('.shade').hide();
					    $('.overBomb').hide();
					    _this.parent('.control').html('<a href="appointment_comment-'+appointment_id+'" class="appraise" orid="'+appointment_id+'">评价</a>');
					    _this.remove();
					    $('.overTip').show();
					    setTimeout(function(){
					    	$('.overTip').hide();

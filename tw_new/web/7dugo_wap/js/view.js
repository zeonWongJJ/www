    ...
    var yanBaotick = function() {
        ...
        function() {
            ...
            if (ybId && ybId != "") {
                ...
                if (ybId == "" && "false" != $("#stockFlag").val()) {
                    $("#directorder").removeClass("btn-fail");
                    $("#directorder").bind("click",
                    function() {
...
function infoRender(b) {
    ...
    if (b && b.stock && b.stock.flag) {
        if ((b.stock.jdPrice > 0) && b.stock.flag) {
            ...
            if ($("#directorder").hasClass("btn-fail") && "" == $("#ybId").val()) {
                $("#directorder").removeClass("btn-fail")
            }
    ...
    if (c) {
        ...
        if (c.proInformation) {
            ...
            if (c.proInformation.activityList) {
                $.each(c.proInformation.activityList,
                function(e, d) {
            ...
            if (c.proInformation.giftList) {
                $.each(c.proInformation.giftList,
                function(e, d) {
                    $("#saleInfo").append('<li> <span class="col02"> <em class="icon-bg02">' + d.text + '</em> <span class="txt01">' + d.value + "</span> </span> </li>")
                })
            }
            if (c.proInformation.suitList && c.proInformation.suitList.length > 0) {
                ...
                var a = "";
                $.each(c.proInformation.suitList,
                function(e, d) {
...
var newAddWare = function(a, e, d, k, b, h, c) {
    ...
    if (isNotBlank(d) && d) {
        ...
        if (k) {
            var g = "/cart/add.json?wareId=" + a + "&num=" + e + "&suitSkuId=" + a + "&suitSkuNum=" + e + (isNotBlank(d) && d ? "&isAjax=" + d: "") + "&sType=8&ybId=" + k + "&resourceType=" + b + "&resourceValue=" + h + (isNotBlank(c) && (c != "") ? "&sid=" + c: "")
        } else {
            var g = "/cart/add.json?wareId=" + a + "&num=" + e + (isNotBlank(d) && d ? "&isAjax=" + d: "") + "&resourceType=" + b + "&resourceValue=" + h + (isNotBlank(c) && (c != "") ? "&sid=" + c: "")
        }
        var j = ajax(g,
        function(n) {
...
var judgeKO = function(d, b) {
    var a = "/ware/judgeOrder.json?wareId=" + d + (isNotBlank(b) && (b != "") ? "&sid=" + b: "");
    var c = ajax(a,
    function(f) {
        try {
            var h = JSON.parse(f);

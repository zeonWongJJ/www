...
function upload_update() {
    ...
    if (yuanimg_str != '') {
        for (var i=0; i<yuanimg_arr.length; i++) {
            ...
            append_yuan += '<img src="/static/style_default/image/up_delete.png" id="delete_'+i+'" onclick="unlink_file('+i+','+"'"+yuanimg_arr[i]+"'"+','+2+','+record_id+')" /><img style="margin-top:7px;" src="/static/style_default/image/up_main.png" id="mainimg_'+i+'" onclick="set_mainpic('+i+','+"'"+yuanimg_arr[i]+"'"+')" />';
            append_yuan += '</div>';

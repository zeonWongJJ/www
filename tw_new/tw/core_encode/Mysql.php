...
class TW_Mysql {
	...
	public function connect($i_index = NULL) {
		...
		if ($this->_b_persistent) {
			try {
				$this->_o_link = new PDO("mysql:host={$this->_s_server};dbname={$this->_s_database}", $this->_s_user, $this->_s_password, array(PDO::ATTR_PERSISTENT => true));
	...
	public function order_by($m_order) {
		...
		if (is_array($m_order)) {
			foreach ($m_order as $s_key => $s_val) {
				if (in_array(strtoupper($s_val), array('ASC', 'DESC'))) {

...
class News_model extends TW_Model
{
    ...
    {
        $a_where    = [
            $this->db->get_prefix() . 'news.news_title LIKE' => '%' . $keywords . '%',
        ];
        $a_where_or = [
            $this->db->get_prefix() . 'news.news_keywords LIKE' => '%' . $keywords . '%',
        ];
        $s_field    = '';
        $a_order    = [
            $this->db->get_prefix() . 'news.news_id' => 'desc',
        ];
        $a_data     = $this->db->from('news')
            ->join('cate', [$this->db->get_prefix() . 'cate.cate_id' => $this->db->get_prefix() . 'news.cate_id'])
            ->where($a_where)
            ->where_or($a_where_or)

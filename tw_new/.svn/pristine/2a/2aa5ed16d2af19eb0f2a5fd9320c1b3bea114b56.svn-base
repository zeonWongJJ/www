...
class TW_Pages {
	...
	*/
	...
	//虚拟
	public function link_style_goods($url, $i_links = 8, $suffix = '.html') {
		$a_link = $this->links($i_links);
		if (empty($a_link['last'])) {
			return '';
		}
		$s_links = '';
		if ($this->_i_page != 1) {
			$s_links = '<a href="' . $url . '1' . $suffix . '" class="pre-page">首页</a>';
			$s_links .= '<a href="' . $url . (($this->_i_page - 1) ? ($this->_i_page - 1) : '1') . $suffix . '" class="pre-page">上一页</a>';
		}
		for ($i = $a_link['start']; $i <= $a_link['last']; $i++) {
			if ($i == $this->_i_page) {
				$s_links .= '<a href="#" class="pre-page"><b>' . $i . '</b></a>';
			} else {
				$s_links .= '<a href="' . $url . $i . $suffix . '" class="pre-page">' . $i . '</a>';
			}
		}
		$s_links .= '<a href="' . $url . ((($this->_i_page + 1) > $this->_i_sum_pages) ? $this->_i_sum_pages : ($this->_i_page + 1)) . $suffix . '" class="pre-page">下一页</a>';
		$s_links .= '<a href="' . $url . $this->_i_sum_pages . $suffix . '" class="pre-page">末页</a>';
		return $s_links;
	}
	/**
	...
	*/
	public function link_style_two($s_links) {

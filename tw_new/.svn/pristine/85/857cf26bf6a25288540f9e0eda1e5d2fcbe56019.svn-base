...
<script>
	...
	export default {
		props: ['taskId', 'type', 'department', 'idsi', 'indexVal', 'ishow', 'divp'],
		data() {
			return {
		...
		methods: {
			...
			liset() {
				let that = this;
				//				console.log('1233333333333333333333333333333333333333333333', that.divp)
				for(var i = 0; i < that.divp.length; i++) {
					if(that.divp[i].task_record_id == that.idsi) {
						that.lists = that.divp[i].sub
												console.log('----211111111111reply_n', that.divp[i].sub)
					}
				}
			},
			//			p评论

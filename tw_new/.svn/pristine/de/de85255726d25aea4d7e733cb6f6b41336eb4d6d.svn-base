	...
	<div class="structure">
		...
		<div id="edit-panel">
			<div>
				...
				<button class="btn" id="btn-insert">发布计划</button>
				<!--月份下拉框-->
				<Dropdown trigger="click" style="margin-left: 20px" @on-click="mothMecp">
					<a href="javascript:void(0)">
						{{firstPlanName}}
						<Icon type="ios-arrow-down"></Icon>
					</a>
					<DropdownMenu slot="list">
						<DropdownItem :name="index" v-for="(item,index) in sinPlan">{{item.plan_name}}</DropdownItem>

					</DropdownMenu>
				</Dropdown>
				<!--计划下拉框-->
				<Dropdown trigger="click" style="margin-left: 20px" @on-click="planMecp">
					<a href="javascript:void(0)">
						{{planName}}
						<Icon type="ios-arrow-down"></Icon>
					</a>
					<DropdownMenu slot="list">
						<DropdownItem :name="index" v-for="(item,index) in planChoice">{{item.name}}</DropdownItem>
...
<script type="text/javascript">
	...
	export default {
		...
		mounted() {
			...
			that.month(2)
			that.$store.commit('monthId', that.Pl);
		},
		methods: {
			task_detail(task_id) {
				let that = this;
				...
				that.axios({
					...
					.then(function(val) {
						if(val.data.error == 0) {
							...
							that.firstPlanName = that.sinPlan[0].plan_name
							that.$store.commit('plan_id', that.sinPlan[0].plan_id);
						} else {
			...
			mothMecp(index) {
				this.firstPlanName = this.sinPlan[index].plan_name;
				this.$store.commit('plan_id', this.sinPlan[index].plan_id);
			},
			planMecp(index) {
				this.planName = this.planChoice[index].name;
				if( this.planName == 公司 ){
					this.Pl=1;
					that.$store.commit('monthId', that.Pl);
				}else{

				...
				<ul>
					<li v-for="(its,index) in list_com" class="list_coms" :class="'state_'+its.order_detail.order_status" @click="detailst(its.order_detail.order_sn)">
						...
						<div class="but_coms" v-if="its.order_detail['order_pay_state_dsc'] == 'PAY_SUCCESS'">
							<div v-if="its.order_detail['order_bis_state_dsc'] == 'PENDING_DOOR' && !fromStaff" class="but_coms_but2" @click.stop="startServer(its.order_detail)">
								开始服务
							</div>
							<div v-if="its.order_detail['order_bis_state_dsc'] == 'PENDING_DOOR' && !fromStaff" class="but_coms_but1" @click.stop="o_cancel(its.order_detail.order_sn)">
								拒绝订单
							</div>

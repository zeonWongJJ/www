...
import bolts.Task;
import com.qidu.chat.helper.CheckSum;
...
public class MethodCallHelper {
  ...
  /*列出自定义表情符号
  * */
    public Task<JSONArray> listEmojiCustom() {
        return call("listEmojiCustom", TIMEOUT_MS, () -> {
            JSONObject param = new JSONObject();
            param.put("msg","method");
            param.put("method", "listEmojiCustom");
            param.put("id", "42");
            param.put("params", new JSONArray());
      ...
      return new JSONArray().put(param);
            ...
            .onSuccessTask(new Continuation<JSONObject, Task<JSONObject>>() {
                public Task<JSONObject> then(Task<JSONObject> task) {
                    ...
                    SharedPreferences pref = context.getSharedPreferences(CommonKey.KEY_LOGIN_USER_ID,MODE_PRIVATE);
  ...
  public Task<Void> loginWithOAuth(final String credentialToken,
    ...
    return call("login", TIMEOUT_MS, () -> new JSONArray().put(new JSONObject()
        .put("oauth", new JSONObject()
            ...
            .onSuccessTask(new Continuation<JSONObject, Task<JSONObject>>() {
                public Task<JSONObject> then(Task<JSONObject> task) {
                    //Log.i("aaaaaa","loginWithOAuth"+task.getResult().toString());
                    //loginWithOAuth{"id":"vsHPi4MHakajy43wg","token":"0Mt9ZY0uKT4aal_raHCfDmThO_Dr1IogrTfzxPZnkxR","tokenExpires":{"$date":1519297485325}}
                    String userId= null;
                    try {
                        userId = task.getResult().getString("id");
                    } catch (JSONException e) {
            ...
            .onSuccessTask(task -> {
                loginWithToken(task.getResult().getString("token"));
                return Task.forResult(task.getResult());
            })
            .onSuccessTask(task -> Task.forResult(task.getResult().getString("token")))
        .onSuccessTask(this::saveToken);
    ...
    return call("logout", TIMEOUT_MS).onSuccessTask(task ->
        realmHelper.executeTransaction(realm -> {
          ...
          /*if (!ConnectivityManager.getInstance(context).getServerList().isEmpty()){
            ...
            DefaultEventEntity obj = new DefaultEventEntity(Constants.IM_LOGIN_OUT);

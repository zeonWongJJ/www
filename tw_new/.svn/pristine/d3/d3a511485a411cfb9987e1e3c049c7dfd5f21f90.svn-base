...
import com.printer.receipt.CashRegisterApplication;
...
//              MsgPrompt.showMsg(mContext, "程序出错啦", msg+"\n点确认退出");
    ...
    private String saveCrashInfoToFile(Throwable ex) {
        ...
        try {
            ...
            String QIDUSTOREDIR="QIDU_DEFAULT_CACHE/";
            String CRASH_LOG="qiduCrashLog/";
            String sdcardDir = state.equals(Environment.MEDIA_MOUNTED) ? Environment.getExternalStorageDirectory().getAbsolutePath()+ File.separator+QIDUSTOREDIR: mContext.getCacheDir().getAbsolutePath()+ File.separator+QIDUSTOREDIR;
            CRASH_LOG=sdcardDir+CRASH_LOG;
            String file_dir = CRASH_LOG;
            ...
            mDeviceCrashInfo.store(trace, "CrashHandler");
            trace.flush();
            trace.close();*/
            return fileName;
        } catch (Exception e) {
    ...
    private String getFilePath() {
        ...
        if (isSDCardExist && isRootDirExist) {
            file_dir = Environment.getExternalStorageDirectory().getAbsolutePath() + "/qiduCrashLog/";
        } else {
            // MyApplication.getInstance().getFilesDir()返回的路劲为/data/data/PACKAGE_NAME/files，其中的包就是我们建立的主Activity所在的包
            file_dir = CashRegisterApplication.getInstance().getFilesDir().getAbsolutePath() + "/qiduCrashLog/";

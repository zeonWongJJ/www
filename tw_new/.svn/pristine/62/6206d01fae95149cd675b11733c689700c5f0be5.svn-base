			...
			if ($come_type == 1) {
				...
				if (!empty($car_have_share)) {
					for ($i=0; $i < count($car_have_share); $i++) {
						...
						if (!empty($score_deduction) && $this_goods_amount > 0) {
							$this_score_deduction = round(($this_goods_amount/$goods_amount)*$score_deduction);
						}
						...
						$a_share_data = [
							...
							'order_state'       => 40,
							'evaluation_state'  => 0,
							'order_message'     => trim($this->general->post('order_message_s')),
				...
				if (!empty($car_not_store)) {
					...
					if (!empty($score_deduction) && $not_goods_amount > 0) {
						$not_score_deduction = round(($not_goods_amount/$goods_amount)*$score_deduction);
					}
					// 抵扣的金额
					if (!empty($score_deduction) && $user_score_cash > 0) {
					...
					$a_not_data = [
						...
						'order_state'       => 40,
						'evaluation_state'  => 0,
						'order_message'     => trim($this->general->post('order_message_0')),
				...
				if (!empty($car_have_store)) {
					for ($i=0; $i < count($car_have_store); $i++) {
						...
						$a_not_data = [
							...
							'order_state'       => 40,
							'evaluation_state'  => 0,
							'order_message'     => trim($this->general->post('order_message_'.$car_have_store[$i])),
							'use_jife'          => $this_score_deduction,
							'use_points'        => $this_use_points,
							'balance_deduction' => $this_balance_deduction,
							'reciver_name'      => $a_address['user_name'],

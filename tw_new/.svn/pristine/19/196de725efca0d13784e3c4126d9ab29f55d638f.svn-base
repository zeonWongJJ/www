				...
				<div class="infoForm">
					<div class="infoBox" v-show="infoShow">
						<van-cell-group>
							...
							<van-field v-if="detailGet.store_user_address_info && detailGet.store_user_address_info.length > 0 && (storeSta == 1 || user_type && user_type != 3)" v-model="detailGet.store_address_info" label="住址地址" disabled />
							...
							<div class="idCard_box">
								<div class="idCardA">
									...
									<ul>
										<li v-if="detailGet.store_id_card_opposite.length > 0">
								...
								<div class="idCardB">
									...
									<ul>
										<li v-if="detailGet.store_id_card_positive.length > 0">
							...
							<div class="qual_box">
								...
								<div class="qualA">
									...
									<ul>
										...
										<li v-if="detailGet.store_zizhi_opposite.length > 0">
								...
								<div class="qualB">
									...
									<ul>
										<li v-if="detailGet.store_zizhi_positive.length > 0">
		...
		methods: { //方法
			init(){
				...
				that.$fetch('user_store_info_get',{}).then(rs =>{
					if( rs.error== 0 ){
						that.user_type = rs.data.staff_row.user_type;
						that.detailGet = rs.data.own_store;
						if(that.statusGet != 0){
							//填充
			...
			loadImg(type, filer) { //图片上传方法
				...
				that.$fetch('uploadBase',lists).then(rs =>{
					if(rs.error == 0) {
						if(type == "idCardA") {
							that.path_imgA = rs.data.path;
						} else if(type == "idCardB") {
							that.path_imgB = rs.data.path;
						} else if(type == "qualA") {
							that.qual_imgA = rs.data.path;
						} else if(type == "qualB") {
							that.qual_imgB = rs.data.path;
						} else if(type == "onStore") {
							that.store_path.push(rs.data.path);
						}

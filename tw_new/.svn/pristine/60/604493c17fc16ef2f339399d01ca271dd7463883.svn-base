...
public abstract class Emojione {
  ...
  public static boolean isCustomEmoji(View view, String input){
    Matcher matcher = SHORTNAME_PATTERN.matcher(input);
    while (matcher.find()) {
      if(SimpleCommonUtils.hostName!=null){
        SharedPreferences sp = view.getContext().getSharedPreferences(CommonKey.KEY_CUSTOM_LIST_EMOJI, Context.MODE_PRIVATE);
   ...
   */
  public static String shortnameToUnicode(TextView textView,String input, boolean removeIfUnsupported) {
    ...
    while (matcher.find()) {
      ...
      /*if (unicode == null) {//必须注释掉，否则自定义表情没走
        continue;
      }*/

      if (supported&&unicode!=null) {
        input = input.replace(":" + matcher.group(1) + ":", unicode);
        return input;
      } else if (!supported && removeIfUnsupported) {
        input = input.replace(":" + matcher.group(1) + ":", "");
        return input;
      }else if(SimpleCommonUtils.hostName!=null&& unicode==null){
        SharedPreferences sp = textView.getContext().getSharedPreferences(CommonKey.KEY_CUSTOM_LIST_EMOJI, Context.MODE_PRIVATE);
        String result = sp.getString("listEmojiCustom", null);
        if(result!=null){
          try {
            JSONArray jsonArray=new JSONArray(result);
            for(int i=0;i<jsonArray.length();i++){
              ...
              if(matcher.group(1).equals(name)){
                ...
                try {
                  ...
                  String path="https://"+ SimpleCommonUtils.hostName+"/emoji-custom/"+nameUTF8+"."+extension;
                  return path;

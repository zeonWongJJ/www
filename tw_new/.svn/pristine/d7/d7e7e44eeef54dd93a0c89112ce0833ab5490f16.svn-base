...
$(function(){
    ...
    var roomsState={
        roomsCategory:false,
        roomsOpenClose:false,
        roomsAddCateName:false,
        roomsCateDesicribe:false
    };
    var cateReg={
        roomsCateName:/^.{1,14}$/,
        roomsCateDesicribe:/^.{1,200}$/
    }
    //二级联动
    ...
    $(".openClose>em").click(function(){
        ...
        roomsState.roomsOpenClose=true;
        return true;
    });
    //添加分类名称
    $("#add_cateName").blur(function(){
        ...
        if( val=="" ){
            ...
            roomsState.roomsAddCateName=false;
            return false;
        }
        if( cateReg.roomsCateName.test(val)){
            $(".addCateName>em").removeClass("hide");
            $(".addCateName>em>img").attr("src","/static/style_default/image/t_03.png");
            $(".addCateName>em>span").html("");
            roomsState.roomsAddCateName=true;
            return true;
        }else{
            $(".addCateName>em").removeClass("hide");
            $(".addCateName>em>img").attr("src","/static/style_default/image/f_03.png");
...
// 中文字符判断
    function getStrLength(str) {
        ...
        roomsState.roomsCateDesicribe=true;
        return reLen;
    }
    $("#cateText").blur(function(){
        ...
        if( val=="" ){
            ...
            roomsState.roomsCateDesicribe=false;
            return false;
        }
    });
    ...
    $("#cateSub").click(function(){
        if( roomsState.roomsCategory && roomsState.roomsOpenClose && roomsState.roomsAddCateName && roomsState.roomsCateDesicribe ){
            $(this).submit();
        }else{
            alert("有选项未选择或格式错误！");
            return false;

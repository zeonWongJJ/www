package com.qidu.chat.layouthelper.chatroom.list
 ...
 */
class RoomMessagesAdapter(private var dataSet: List<Message>, private val hostname: String, private val context: FragmentActivity) : RecyclerView.Adapter<RoomMessagesAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.messageBody.setText(context,message,position,false)

        val webContents = message.webContents
        if (webContents == null || webContents.isEmpty()) {
            holder.messageUrl.visibility = View.GONE
        } else {
            holder.messageUrl.setUrls(context,message.webContents, true,false)
        }

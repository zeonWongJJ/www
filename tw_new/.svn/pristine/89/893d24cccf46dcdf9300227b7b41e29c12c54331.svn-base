...
<html>
	...
	<body>
        ...
        <!-- 拉框结束 -->
		<div class="main">
			<header class="head"><a href="nuser_center"><img src="static/style_default/images/gouxiang_18.png"/></a><i>座位订单</i></header>
			<div class="wrapper wrapper01" id="retr">
				<div class="scroller">
					<ul class="clearfix">
						...
						<li <?php if ($a_view_data['state'] == 2) { echo 'class="current"'; } ?>><a href="book_order-2">已接单</a></li>
						<li <?php if ($a_view_data['state'] == 3) { echo 'class="current"'; } ?>><a href="book_order-3">进行中</a></li>
				...
				<ul>
					...
					<!-- 	<div class="control clearfix" id="ctrl_<?php echo $value['appointment_id']; ?>">
							<?php if ($value['appointment_state'] == 0) {
								...
								echo '<a href="office_detail-'.$value['office_id'].'" class="bookAgain" onclick="book_again('.$value['office_id'].')">再次预约</a>';
							} else if ($value['appointment_state'] == 6) {
								echo '<a href="office_detail-'.$value['office_id'].'" class="bookAgain" onclick="book_again('.$value['office_id'].')">再次预约</a>';
							} ?>
						</div> -->

						<!---状态改版除了未支付和未接单的，已入店和点餐合并-->
						<div class="control clearfix" id="ctrl_<?php echo $value['appointment_id']; ?>">
							<?php if ($value['appointment_state'] == 0) {
								echo '<a onclick="book_cancel('.$value['appointment_id'].')">取消订单</a>';
								echo '<a class="bookAgain" onclick="go_pay('.$value['appointment_id'].','.$value['actual_pay'].')">去付款</a>';

...
/************************************* 添加权限 *************************************/
	...
	public function auth_add() {
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			$auth_type = trim($this->general->post('auth_type'));
			...
			//验证数据权限是否已经存在
			$check_auth_url = $this->auth_model->get_auth_exist($auth_url, $auth_type);
			if ($check_auth_url) {
	...
	public function auth_update() {
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if ($i_result) {
				...
				if ($auth_url != $original_url) {
					//所有角色
					$a_data_role = $this->auth_model->get_role_info();
					foreach ($a_data_role as $key => $value) {
						$role_auth = $value['role_auth'];
						if (strpos($role_auth, $original_url) !== false) {
							$role_auth = str_replace($original_url, $auth_url, $role_auth);
							$a_where = [
							...
							$i_result = $this->auth_model->update_role_all($a_where, $a_data);
						}
					}
...
/************************************* 删除权限 *************************************/
	...
	public function auth_delete() {
		...
		if ($type==1) {
			...
			if ($i_result) {
				//删除成功后将所有角色里该权限删除
				$this->update_role_auth(1, $auth_id, $auth_url);
				$this->update_role_auth(2, $auth_id, $auth_url);
...
/*********************************** 更新角色权限 ***********************************/
	...
	public function update_role_auth($type, $auth_id, $auth_url) {
		if  ($type==1) {
			$a_data_role = $this->auth_model->get_role_info();
			foreach ($a_data_role as $key => $value) {
				...
				if (in_array($auth_id, $auth_ids)) {
					...
					for ($i=0; $i<count($role_auth); $i++) {
						if ($role_auth[$i]==$auth_url) {
							unset($role_auth[$i]);
						}
					}
					$auth_count = count($auth_ids);
					$role_auth  = implode('-', $role_auth);

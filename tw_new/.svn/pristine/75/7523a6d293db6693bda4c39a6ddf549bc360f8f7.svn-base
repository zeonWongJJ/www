<template>
	<div class="sub_eval">
		...
		<div class="divp_box" v-for="(itek,index) in lists_plan">
			<div></div>
			<div class="divp">
				<p style="color:white;">{{itek.my_name}} 回复 {{itek.reply_name}} <span style="font-size: 12px; color: #999;"> {{itek.add_date}} </span></p>
				<div style="display: flex;justify-content: space-between;align-items: baseline;">
					<div style="margin: 20px 0 0 0;flex: 0 0 700px; padding:8px 5px;border-radius: 5px;">
						<pre style="width: 700px;color:rgb(112, 112, 112); white-space: pre-wrap;word-wrap: break-word;">{{itek.content}}</pre>
					</div>
		...
		<div class="evaluate" v-show="!indexVal">
			<div class="evaluate_top">
				<div>
					<input ref="postcontent" rows="2" placeholder="请输入" v-model="texte" @click="isshow = true">
				</div>
			</div>
			<div>
				<button v-show="!isshow">回复</button>
				<button v-show="isshow" @click="oshow(),tijiao()">回复</button>
			</div>
		</div>

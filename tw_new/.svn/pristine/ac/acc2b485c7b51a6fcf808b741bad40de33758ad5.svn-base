					...
					<ul>
						<li v-for="(its,index) in list_com" class="list_coms" @click="detailst(its,index)">
							...
							<div class="but_coms" style="display: flex; justify-content:flex-end; align-items:center">
								<div v-if="its.order_detail.order_pay_state_dsc == 0 && its.order_detail.order_state == 0 || its.order_detail.order_state == 1 || (its.order_detail.order_state == 2 && its.order_detail.order_rate == 0)" class="but_coms_but1"
								...
								<template v-if="its.order_detail['order_pay_state_dsc'] == 'PAY_SUCCESS'">
									<div v-if="its.order_detail.order_state == 2 || its.order_detail.order_state == 1 && its.order_detail.order_rate == 0"
										 class="but_coms_but4"

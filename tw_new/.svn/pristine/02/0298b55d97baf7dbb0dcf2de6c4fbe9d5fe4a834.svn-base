 ...
 * 例：
	...
	$a_data = $this->_i_pages->get(100, 1, 5);
	print_r($a_data);
	生成链接：echo $this->_i_pages->link_style_one(get_config_item('domain') . '/item-');
*/
class Pagess {
	...
	public function link_style_one($url, $i_links = 8, $suffix = '.html') {
		...
		for ($i = $a_link['start']; $i <= $a_link['last']; $i++) {
			if ($i == $this->_i_page) {
				...
				$s_links .= "<input id='a' onclick='fi(this)' type='button' name='i' value='$i'>";
			} else {
				//$s_links .= '<a href="' . $url . $i . $suffix . '">' . $i . '</a>';
				//$s_links .= "<input id='b' onclick='fi(this)' type='button' name='i' value='$i'>";
				$url = $url . $i . $suffix;
				$s_links .= '<a href="javascript:fu($url . $i . $suffix)">' . $i . '</a>';
			}
		}

			...
			<div v-for="item in addslists">
				...
				<div class="but_coms" v-if="item.order_detail">
					...
					<template v-if="item.order_detail['order_pay_state_dsc'] == 'PAY_SUCCESS'">
						...
						<div v-if="item.order_detail['order_bis_state_dsc'].indexOf(['SET_UP','PENDING_PAY', 'PENDING_ORDER', 'PENDING_ASSIGN', 'PENDING_DOOR']) && item.order_detail['order_bis_state_dsc'] != 'COMPLETED' && item.order_detail['order_bis_state_dsc'] != 'CLOSED'" class="but_coms_but1" @click.stop="shows(item)">取消订单</div>
					</template>
					<template v-else>

		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if ($pay_type == 1) {
				...
				$a_param = [
					...
					'total_fee' => $recharge_money*100,
					// 异步通知URL，必填
					'notify_url' => 'http://www.7dugo.com/recharge_wxpaynot.html',
					...
					// 'url_fail' => 'http://www.7dugo.com/recharge_wxpaynot.html'
				];
...
/********************************* 微信异步 *********************************/
	...
	public function recharge_wxpaynot() {
		...
		// 验证数据安全方法一：(签名验证)
		$i_result = $this->db->insert('test', ['test_content'=>json_encode($_POST)]);

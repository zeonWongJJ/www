...
class Account_model extends TW_Model
{
    ...
    {
        $s_field = 'account_id, order_count, product_count, money_count, account_time, account_state, money_update, remark_update, ' . $this->db->get_prefix() . 'account.store_id, store_linkman, store_contact, store_name, order_ids, is_correct';
        $a_order = [
            'account_id' => 'desc',
        ];
        $a_data  = $this->db->from('account')
            ->join('store', [$this->db->get_prefix() . 'account.store_id' => $this->db->get_prefix() . 'store.store_id'])
    ...
    public function get_account_page($state, $time)
    {
        ...
        $s_field           = 'account_id, order_count, product_count, money_count, account_time, account_state, money_update, remark_update, ' . $this->db->get_prefix() . 'account.store_id, store_linkman, store_contact, store_name, order_ids, is_correct, month_score,coffee_ordercount,office_ordercount';
        $a_order           = [
            'account_id' => 'desc',
        ];
    ...
    public function get_account_one($account_id)
    {
        ...
        $a_order = [];
        $s_field = 'account_id, order_count, product_count, money_count, account_time, account_state, money_update, remark_update, ' . $this->db->get_prefix() . 'account.store_id, store_linkman, store_contact, store_name, store_address, order_ids, is_correct, office_order';
        $a_data  = $this->db->from('account')
            ->join('store', [$this->db->get_prefix() . 'account.store_id' => $this->db->get_prefix() . 'store.store_id'])

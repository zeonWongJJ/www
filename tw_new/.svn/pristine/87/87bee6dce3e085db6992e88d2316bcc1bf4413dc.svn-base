...
class Bill_ctrl extends TW_Controller {
	...
	//新版的结算页面
	public function n_bill() {
		// 获取POST数据将其加入订单
		if ( ! empty($_POST)) {
			$come_type = trim($this->general->post('come_type'));
			$distribution_type = trim($this->general->post('distribution'));
			$a_data['a_cart'] = $this->general->post();
			$_SESSION['post'] = $a_data['a_cart'];
		} else {
			$a_data['a_cart'] = $_SESSION['post'];
			$come_type = $_SESSION['post']['come_type'];
			$distribution_type = $a_data['a_cart']['distribution'];
		}
		// var_dump($a_data['a_cart']);exit;
		if ($come_type == 1 || $come_type == 4) {
			...
			// 验证数据
			if (empty($cart_ids)) {
				$this->error->location($this->router->url('index'));
			}
			$store_arr   = array();
			$share_arr   = array();
			$nostore_arr = array();
			if (!empty($cart_ids)) { 
				...
				if (!empty($store_arr)) {
					for ($i=0; $i < count($store_arr); $i++) {
						$a_this_store = $this->bill_model->get_store_one($store_arr[$i]);
						// 此门店的运费
						...
						foreach ($a_cart_part as $key => $value) {
							...
							$a_data['goods']['store'][$i]['store_name'] = $a_this_store['store_name'];
                ...
                if(!empty($store_arr)){
                    ...
                    $appointment_str = $this->bill_model->get_seat_price($_SESSION['user_id'], $store_arr);
                    $a_data['appointment_id_str'] =$appointment_str['appointment_id_str'];
                    $a_data['order_price']        = ($goods_amount_total + $good_freight_total) - $appointment_str['actual_pay'];
                }else{
				...
				$a_data['come_type'] = 1;
				$a_data['distribution_type'] =$distribution_type;
				$a_data['shipping_fee'] = $user_order_freight;
//                var_dump($a_data['appointment']['appointment_price']);exit;
				$this->view->display('n_new_bill', $a_data);
			}
		} else if ($come_type == 2) {
			// 立即购买
			if ( ! empty($_POST)) {

...
public abstract class CordovaWebActivity<T extends BasePresenter> extends BaseActivity<T> implements CordovaInterface{
    ...
    protected void initWebview() {
        ...
        progressbar.setLayoutParams(new FrameLayout.LayoutParams(FrameLayout.LayoutParams.FILL_PARENT, 8));
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
            ...
            webViewClient = new IceCreamCordovaWebViewClient(this, webView) {
                ...
                public void onPageFinished(WebView view, String url) {
                    super.onPageFinished(view, url);
                    isFinish=false;
                    //https://vdao-mobile.7dugo.com/login
                    /*Log.i("aaaaaaa",url);
                    if(url.equals("https://vdao-mobile.7dugo.com/nuser_center")){
                        Log.i("bbbbbb",url);
    ...
    public Object onMessage(String id, Object data) {//这里接收，postMessage发送
        ...
        if ("onPageFinished".equals(id)) {
            ...
            finish();
        }/*else if(PluginAction.ACTION_TITLE_BAR_LAYOUT_IS_VISIBLE.equals(id)){
            JSONObject d = (JSONObject) data;
            try {
                final int isShowTitle=d.getInt("isShowTitle");
...
//				}
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        Log.i("aaaaaaaa", "dispatchKeyEvent: keyCode -- " + event.getKeyCode());
        if (event.getKeyCode()==4&&isFinish){//回退键的KeyCode是4.
            finish();
            return false;//表示不分发
        }else{

	...
	<div class="details">
		...
		<div class="body">
			...
			<div class="detail">
				...
				<div class="other">
					<div class="start">
						<van-rate :value="Number(items.service_average_score)" disabled-color="#ff3434" void-color="#ceefe8" disabled/>
		...
		methods: {
			...
			rendDetails() { //详情信息
				this.$fetch('service_get', {}, this.serverId).then(rs =>{
					if(rs.error == 0) {
						this.images = this.images.concat(rs.data.service_img);
						this.items = rs.data;
						this.comment_count = rs.data.comment_count;
						this.user_collected = rs.data.user_collected
						this.comment_list()
					} else {

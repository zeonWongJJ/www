					...
					<li v-for="(its,index) in list_com" class="list_coms" @click="detailst(its,index)">
						...
						<div class="but_coms">
							...
							<template v-if="its.order.order_detail['order_pay_state_dsc'] == 'PAY_SUCCESS'">
								...
								<div v-if="its.order.order_detail['order_bis_state_dsc'].indexOf(['SET_UP','PENDING_PAY', 'PENDING_ORDER', 'PENDING_ASSIGN', 'PENDING_DOOR'])" class="but_coms_but1" @click.stop="shows(its,index)">取消订单</div>
							</template>
							<template v-else>
							...
							<div v-else-if="its.order.order_detail['order_bis_state_dsc'] == 'COMPLETED' || its.order.order_detail['order_bis_state_dsc'] == 'CLOSED'" class="but_coms_but1" @click.stop="shows_qx(its,index)">删除订单</div>
						</div>

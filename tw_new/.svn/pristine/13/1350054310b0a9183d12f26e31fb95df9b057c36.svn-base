		...
		<div class="body">
			...
			<div class="top">
				<div class="img" @click="revise" v-if="user_type == 3">
					<img :src="store.store_pic[0].length == 0 ? defaultImg : uploadFileUrl + store.store_pic[0]"/>
			...
			init(){
				...
				that.$fetch('user_store_info_get', {}).then(rs =>{
					if( rs.error== 0 ){
						...
						that.staff_id = rs.data.staff_row.user_id;
						if(that.user_type == 3){//店主
							that.store = rs.data.own_store;
						}else{//店员
							that.store = rs.data.superior;
						}
					}else{
						that.$toast(rs.msg[0]);
					}

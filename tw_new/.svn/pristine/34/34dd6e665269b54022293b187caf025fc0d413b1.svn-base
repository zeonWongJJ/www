...
//            $this->db->begin();
        ...
        if ($data_set === 'list') {
            ...
            $rows           = $query->limit($offset * 1 < 0 ? 0 : $offset, $limit)
                ->where($this->condition)
                ->order_by($sort)
                ->get([$this->table => 'a']);
            $list_query_sql = APP_DEBUG ? $this->db->get_sql() : '';
            ...
            if (method_exists($this, 'getListCount')) {
                ...
                foreach ($this->condition as $field => $value) {
                    if (strpos($field, '.') > -1) {
                        ...
                        if ($explode[0] == 'a') {
                            $count_condition[$field] = $value;
                        }
                    }
                }
                $count = $this->db->get_total([$this->table => 'a'], $count_condition);
            }
            ...
            // 计算分页
            $_rows = app('request')->post('rows', 30, 'intval');
            // 页数 = 总数 / 每页多少
            $pages = ceil($count / $_rows);
            // 当前页
            $_page = app('request')->post('page', 0, 'intval') ?: 0; // 当前页面计算
            if (!$_page) {
                $limit = app('request')->post('limit', 0, 'intval') ?: 1; // 偏移量
                $_page = ceil($limit / $_rows);
            }
...
//        }
    ...
    public function delete($id)
    {
        try {
            $this->db->begin();
            $this->db->set_error_mode();
            $is_soft_delete = app('request')->post('soft_delete', 0, 'intval');

            // 不允许删除存在下级的节点
            if (\is_array($id)) {
                $id_implode = implode(',', $id);
                $sql        = <<<EOF
SELECT * FROM {$this->db->get_prefix($this->table)} WHERE {$this->pk_id} IN ({$id_implode})
EOF;
                ...
                if (!$is_soft_delete) {
                    $sql = <<<SQL
DELETE FROM {$this->db->get_prefix($this->table)} WHERE {$this->pk_id} IN ({$id_implode});
SQL;
                } else {
                    $sql = <<<SQL
UPDATE {$this->db->get_prefix($this->table)} SET `is_del` = 1 WHERE {$this->pk_id} IN ({$id_implode})
SQL;
                }

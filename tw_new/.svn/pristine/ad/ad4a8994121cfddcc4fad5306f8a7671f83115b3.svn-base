		...
		<div v-else class="orderd_box">
			...
			<div class="order_but">
				<template v-if="usertype == 3">
					<div v-if="addslists.order_detail.order_is_peddling == 0 && addslists.order_detail.order_state == 0 || addslists.order_detail.order_state == 1 || addslists.order_detail.order_state == 2" class="but_coms_but1" @click.stop="o_cancel(addslists)">取消订单</div>
					<div v-if="addslists.order_detail.order_is_peddling == 0 && addslists.order_detail.order_state == 1 && addslists.server_info.appointed_uid == 0" class="but_coms_but2" @click.stop="sureOrder(addslists)">确认接单</div>
					<div v-if="addslists.order_detail.order_is_peddling == 0 && addslists.server_info.appointed_row.length > 0 && addslists.order_detail.order_rate == 0  && addslists.time_record.order_sm_at == 0" class="but_coms_but2" @click.stop="xg_distri(addslists)">修改分配</div>
					<div v-if="addslists.order_detail.order_is_peddling == 0 && addslists.server_info.appointed_row.length == 0 && addslists.order_detail.order_rate != -1 && addslists.order_detail.order_state == 2" class="but_coms_but2" @click.stop="xg_distri(addslists)">分配人员</div>
					<div v-if="canIreceipt && addslists.order_detail.order_is_peddling == 0 && addslists.order_detail.order_state == 2 && addslists.server_info.appointed_uid == user_id" class="but_coms_but2" @click.stop="startServer(addslists.order_detail)">开始服务</div>
					<!--<div v-if="addslists.order_detail.order_state == 4 || addslists.order_detail.order_state == 5" class="but_coms_but1" @click.stop="shows(addslists)">删除订单</div>-->
					<div v-if="addslists.order_detail.order_rate == 0 && addslists.order_detail.order_state == 3" class="but_coms_but2" @click.stop="completed(addslists)">已完成</div>
				</template>
				<template v-else>
					<div v-if="addslists.order_detail.order_is_peddling == 0 && addslists.order_detail.order_state == 2" class="but_coms_but2" @click.stop="startServer(addslists.order_detail)">开始服务</div>
					<div v-if="addslists.order_detail.order_is_peddling == 0 && addslists.order_detail.order_state == 2" class="but_coms_but1" @click.stop="o_cancel(addslists)">拒绝订单</div>
					<div v-if="addslists.order_detail.order_is_peddling == 0 && addslists.order_detail.order_state == 3 && addslists.order_detail.order_rate == 0" class="but_coms_but2" @click.stop="completed(addslists)">已完成</div>
				</template>
				<div v-if="addslists.order_detail.order_is_peddling == 0 && addslists.order_detail.order_rate == 1 && addslists.order_detail.order_comment_id != 0" class="but_coms_but2" @click.stop="getComment(addslists.order_detail.order_comment_id)">查看评价</div>
			</div>
		</div>
		<!--取消订单-->

...
<body>
        ...
        <?php if ( ! empty($a_view_data)) {?>
		 <ul>
			<?php foreach ($a_view_data as $val) {
                ?>
            <li>
                <a href="goods-<?php echo $val['fav_id'] ?>-1" class="mf-item clearfix">
                    <span class="mf-pic">
                    ...
                    <div class="mf-infor">
                        ...
                        <p class="mf-pd-price">￥<?php echo $val['goods_price'] ?></p>
                        
                        
                    </div>
                </a>
                <span class="i-cart" value="<?php echo $val['fav_id'] ?>" style="background:url(../image/cart.png) no-repeat 4px 5px;width: 39px;height: 23px;float: left;"></span>
 ...
 $(".i-del").unbind("click").click(function(){
    ...
    $.ajax({
        type : "POST",
        url : "<?php echo $this->router->url('favorites');?>",
        data: "id="+id,
        dataType : "json",
        ...
        { 
            if(data == 1){
                alert("删除成功！");
                self.location='<?php echo $this->router->url('favorites');?>';
            }
        }
    })
 })

...
<script>
    function to_register() {
        getGenerateDeviceUniqueId(function(deviceId) {
            $.ajax({
                url: '<?php echo $this->router->url('key_register'); ?>',
                type: 'post',
                data : {id_equipment:deviceId},
                dataType: 'json',
    ...
    function to_mobli() {
        ...
        showPhoneInputEdit(function(phoneNumber) {
            //获得手机号码phoneNumber之后进行你的操作
            // alert(phoneNumber);
            $.ajax({
                type : 'post',
                url : '<?php echo $this->router->url('mobile');?>',
                // data : {mobile:phoneNumber,user_name:name,randpwd:randpwd},
    ...
    function login_user() {
        ...
        getGenerateDeviceUniqueId(function(deviceId) {
            $.ajax({
                ...
                data : {username:name,passwd:randpwd,id_equipment:deviceId},
                dataType : 'json',
                success : function(data) {
                  if (data.code == 23) {
                    alert('请填写用户名和密码！');
                  ...
                  var json={"memberId":data.memberId,"memberName":data.memberName,"equipmentId":data.equipmentId};
                  loginSuccess(json);                              
                }
            })
        });
    ...
    $('.oImage').click(function(){
        takeLocalUserList(function(userArray){
            var userArray = JSON.parse(userArray);
            var id = $(".username p").text();
            if (id == "") {
                $.each(userArray, function (n, value) {
                   $(".username").append("<p>"+value.memberName+"</p>");                   
               });
            };  
            //用户选择

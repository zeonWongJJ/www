...
class Pay_model extends TW_Model {
	...
	public function pay(){
		...
		// 1、余额支付2、货到付款3、支付宝支付
		$out_trade_no = $this->payment($pay_sn_sum,$s_paytype);
		...
		$a_param = [
			...
			// 订单名称，必填
			'subject' => rtrim($s_gname, ','),
			// 付款金额，必填
			'total_amount' => $a_data['order_amount'],
			//'total_amount' => 0.01,
			// 商品描述，可空
			'body' => '7度购健康商城',
		...
		echo $this->alipay_wap->pay($a_param);
	}
...
/***************************************** 旧代码 *************************************************/
		...
		// 条件
		$a_data = $this->db->where_in('pay_sn', $pay_sn)->get('order', '','order_amount,order_sn');
		foreach ($a_data as $key => $value) {
			$price_sum += $value['order_amount'];

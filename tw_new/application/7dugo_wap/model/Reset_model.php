<?php
class Reset_model extends TW_Model {
	...
	public function get_username($username) {
		// $a_where = [ 'member_name' => $username, 'member_mobile' => $username]; 
		$a_where = ['member_mobile' => $username]; 
		$s_field = 'member_name'; 
		$s_data = $this->db->get_row('member',$a_where,$s_field);
		$s_data = $this->db->where_or($a_where)->get_row('member', '', $s_field);
		if( $s_data !== false ){

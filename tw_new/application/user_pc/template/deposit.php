...
<div class="ordertime-cont">
  <span id="time-txt">
        ...
        <ul id="allTime">
            <li><a href="<?php echo $this->router->url('deposit', ['while'=>'0', $a_view_data['i_page']]);?>" >近三个月收支明细
            </a></li>
            <li><a href="<?php echo $this->router->url("deposit", ['while'=>'1', $a_view_data['i_page']]);?>" >今年内收支明细</a></li>
            <li><a href="<?php echo $this->router->url("deposit", ['while'=>'2', $a_view_data['i_page']]);?>" >2016年收支明细</a></li>
            <li><a href="<?php echo $this->router->url("deposit", ['while'=>'3', $a_view_data['i_page']]);?>" >2015年收支明细</a></li>
            <li><a href="<?php echo $this->router->url("deposit", ['while'=>'4', $a_view_data['i_page']]);?>" >2014年收支明细</a></li>
            <li><a href="<?php echo $this->router->url("deposit", ['while'=>'5', $a_view_data['i_page']]);?>" >2014年之前收支明细</a></li>
        </ul>
</div>
  ...
  <?php if (empty($a_view_data['res']['thre'])) {?>
      <div class="product">
                        <ul>
                            <span class='t'>暂无提现记录.....</span>     
                        </ul>
                    </div>
    <?php } else {?>
   <table id="table_bg">
     <tbody>
     <?php foreach ( $a_view_data['res']['thre'] as $value ) { ?>
       <tr>
         <td style="width: 150px; "><?php echo date('Y-m-d H:s A' , $value['pdc_time_create']); ?></td>
         <td style="width: 100px; ">
         <div class="td_bg1" style="background-color:white">提现</div>
         </td>
         <td style="width: 100px; "><div class="autocut width100">-<?php echo $value['pdc_amount']; ?></div></td>
         <td style="width: 200px; "><div class="autocut width200">提现到<?php echo $value['pdc_bank_name']; ?>银行卡</div></td>
         <td><span class="verticalline"></span></td>
         <td style=" width: 200px; text-align:left; "><div class="autocut width200">
         <?php if( $value['pdc_payment_state'] ){
              echo '已于'.date('Y-m-d H:s A' ,$value['pdc_payment_time']).'到账';
         } else {
              echo '预计'.date('Y-m-d H:s A' , $value['pdc_payment_time']+7200).'前到账';
         } ?>
         </div></td>
         <td style="padding-left: 20px;">
         <div class="td_bg2">
         <div class="uptd_bg2" style="background-color:white; width:<?php if( $value['pdc_payment_state'] ){
          echo '200';
         }else{
          echo intval($_SERVER['REQUEST_TIME']/($value['pdc_time_create']+7200))*200;
         } ?>px;">
         </div></div>
         </td>
       </tr>

...
/************************************* 附近门店 *************************************/
	...
	public function store_showlist() {
		if ($_SERVER['REQUEST_METHOD'] == 'GET') {
			$a_data = $this->store_model->get_stroe_nearby();
		...
		if ($_SERVER['REQUEST_METHOD'] == 'GET') {
			...
			if (empty($a_data)) {
				...
				foreach ($a_data as $key => $value) {
					...
					$value['go_url'] = get_config_item('domain').'/store_detail-'.$value['store_id'];
					if (empty($value['store_mainimg'])) {
						$value['main_pic'] = get_config_item('store_touxiang').'upload/store/20171216060310860.png';
					} else {
...
/********************************** 共享办公室详情 **********************************/
	...
	public function office_detail() {
		if ($_SERVER['REQUEST_METHOD'] == 'GET') {
			...
			// 展示页面
			$this->view->display('office_detail2', $a_data);
		}
	}

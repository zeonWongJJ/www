      ...
      <div class="content">
        <div class="left">
          <div class="img" @click="pers">
            <img :src="user.user_pic.length == 0 ? imgs : user.user_pic" />
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      init() {
        var that = this;
        that.$fetch('user_info_get', {}, '', 'GET')
        .then(rs =>{
        	if (!rs.error) {
            that.user = rs.data;
            that.$store.commit('userPhone', rs.data.user_phone);
          } else {
            that.$toast(rs.msg[0])
          }
        })
        that.$fetch('user_order_statistics', {})
        .then(rs =>{
        	if (rs.error == 0) {
        ...
        that.$fetch('user_store_info_get', {})
        .then(rs =>{
        	if (rs.error == 0) {
            that.store_type = rs.data.staff_row.user_type;
          } else {
//          that.$toast(rs.msg[0])
          }
        })
      ...
      goStore() { //店铺
        ...
        .then(rs =>{
          if(rs.error == 0){
          	...
          	that.$toast(JOSN.stringify(rs.msg))
          }
        })
        //
      },

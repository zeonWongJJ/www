  ...
  export default {
    ...
    methods: {
      initCateList() {
        ...
        that.$fetch('category_list', lists)
        .then(rs =>{
        	if(rs.error == 0){
        		localStorage.setItem('cache.home.cate.list', JSON.stringify(rs.data));
            that.home_cate_list = rs.data
        	}else{
        		that.$toast(rs.msg[0]);
        	}
        })
      ...
      init() {
        //存储用户店铺状态
        var that = this;
        that.$fetch('user_store_status', {}, '', 'GET')
        .then(rs =>{
        	if(rs.error == 0){
        ...
        data.filter = '1'
        that.$fetch('slide_list', data)
        .then(rs =>{
        	if(rs.error == 0){
            localStorage.setItem('cache.images.list', JSON.stringify(rs.data))
        		that.images = rs.data;
        	}else{
        		that.$toast(JSON.stringify(rs.msg))
        	}
        ...
        .then(rs =>{
        	if(rs.error == 0){
        		localStorage.setItem('cache.list.com', JSON.stringify(rs.data))

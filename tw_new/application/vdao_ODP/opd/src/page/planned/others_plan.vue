	...
	<div class="plan">
		<div class="tit_box">
			<ul class="plan_tit_box_ul">
				<li @click="plan">我的计划</li>
				<li @click="participate_plan">参与计划</li>
				<li class="tit_box_li">他人计划</li>
			</ul>
		</div>
		<div class="myplan test-1">
			...
			<div class="ul_div" v-for="item in myplan" v-else>
				<div class="li_div" @click="plan_details(item,index)">
					<div>
						<div class="plantype">
							<img v-if="item.plantype == 1 && item.thisWeek" src="../../assets/img/week.png" />
						...
						<div class="progressBox">
							<span class="person">{{item.planPerson}}</span>
							<template>
								<div class="progress">
	...
	export default {
		...
		methods: {
			...
			plan_details(item, index) {
				let that = this
				let oid_details = item.plan_id
				let plan_score = item.total_score
				this.$router.push({
					path: 'plan_details',
					query: {
						oid_details,

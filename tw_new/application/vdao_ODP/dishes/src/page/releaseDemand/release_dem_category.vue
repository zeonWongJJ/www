<template>
	<div class="squared">
		<div>
			<van-nav-bar title="选择需求类别" left-arrow @click-left="onClickLeft" @click-right="onClickRight">
				<van-icon name="wap-nav" color="black" slot="right" />
			</van-nav-bar>
		...
		<div class="div_box">
			<ul>
				<li class="div_box_li" v-for="items in list" v-if="list">
				...
				<li v-if="list == ''">
					<div style="text-align: center;margin: .35rem 0 0 0; color: #b2b2b2;">
						暂无项目
					</div>
				</li>
	...
	export default {
		...
		methods: { //方法
			...
			list_posh() {
				...
				this.$fetch('category_list', lists, '-' + this.$route.query.ids).then(rs =>{
          this.list = rs
          if(rs.length < 1) {
            this.$router.push({

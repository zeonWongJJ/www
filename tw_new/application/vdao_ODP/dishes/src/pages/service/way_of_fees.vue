				...
				<div class="table">
					...
					<div class="tr unit">
						...
						<div class="td" @click="typeShow(2)">
							...
							<div class="more" v-else>{{selected.unit.id ? selected.unit.name : '请选择'}}</div>
						</div>
					</div>
				</div>
		...
		<van-popup v-model="show" position="bottom">
		 	<div class="popup">
			 	...
			 	<div class="ul">
			 		...
			 		<div v-show="popupType == 2" class="li" :class="{check : (popupData.id ? popupData.id : selected.unit.id) == item.id}" v-for="item in unitList" @click="popupData = item">{{item.name}}</div>
			 	</div>
		 	</div>
		</van-popup>
	</div>

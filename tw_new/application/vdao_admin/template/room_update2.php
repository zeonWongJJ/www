...
<head lang="en">
    ...
    <script src="./static/style_default/script/releaseRoomsupdate.js"></script>
    <title>修改房型</title>
</head>
...
<div class="productClassification">
    ...
    <article>
        ...
        <div class="releaseRooms">
            <p><a href="room_showlist" style="color:#000;">房间管理</a> > 修改房间</p>
        ...
        <div class="roomsList">
            <form action="room_update" method="post">
                <input type="hidden" id="record_id" name="room_id" value="<?php echo $a_view_data['detail']['room_id']; ?>">
                <ul>
                    <li class="roomName">
                        <em><s>*</s>房间名称</em>
                        <input type="text" id="room_name" name="room_name" value="<?php echo $a_view_data['detail']['room_name']; ?>" />
                        <span class="hide">
                    ...
                    <input type="hidden" name="room_state" value="<?php echo $a_view_data['detail']['room_state']; ?>">
                    <li class="roomDisplay">
                        <em><s>*</s>是否开放</em>
                        <em class="sure" style="width:50px; text-align:left;" value="1">
                            <?php if ($a_view_data['detail']['room_state'] == 1) {
                        ...
                        <em  class="deny" style="width:50px; text-align:left;" value="0">
                            <?php if ($a_view_data['detail']['room_state'] == 0) {
                                echo '<img src="./static/style_default/image/pro_36.png" />';
                            } else {
                                echo '<img  src="./static/style_default/image/pro_38.png" />';
                            } ?>
                    ...
                    <li class="roomSeat">
                        <em><s>*</s>座位</em>
                        <input type="text" id="room_seat" name="room_seat" onkeypress="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value" onkeyup="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value" onblur="if(!this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?|\.\d*?)?$/))this.value=this.o_value;else{if(this.value.match(/^\.\d+$/))this.value=0+this.value;if(this.value.match(/^\.$/))this.value=0;this.o_value=this.value}" value="<?php echo $a_view_data['detail']['room_seat']; ?>" />
                    ...
                    <li class="roomKeyWord">
                        <em><s>*</s>房间关键字</em>
                        <input type="text" id="key_word" name="room_keywords" value="<?php echo $a_view_data['detail']['room_keywords']; ?>" />
                         <span class="hide">
                            <img src="./static/style_default/image/t_03.png" />
                            <em></em>
                        </span>
                    ...
                    <li class="mainFigure">
                        ...
                        <div class="figureContent">
                            ...
                            <input type="hidden" name="mainpic_path" value="<?php echo $a_view_data['detail']['room_mainpic']; ?>">
                            <input type="hidden" name="otherpic_path" value="<?php echo $a_view_data['detail']['room_otherpic']; ?>">
                            <span>请至少上传一张产品图片；支持jpg/png格式，单张（长&lt;xxx，宽&lt;xxx，大小<1M），
最多支持10张图片，将按上传顺序展示图片，支持批量上传。</span>
                        </div>
                    </li>
                    <input type="hidden" name="device_ids" value="<?php echo $a_view_data['detail']['device_ids']; ?>">
                    <li class="roomsDevice">
                        <em>配备设备</em>
                        <div class="deviceBox">
                            <?php foreach ($a_view_data['device'] as $key => $value): ?>
                            <a value="<?php echo $value['device_id']; ?>" <?php $ids = explode(',', $a_view_data['detail']['device_ids']); if (in_array($value['device_id'], $ids)) { echo 'class="check"'; }  ?> >
                                <span><?php echo $value['device_name']; ?></span>
                                <?php
                                    $ids = explode(',', $a_view_data['detail']['device_ids']);
                                    if (in_array($value['device_id'], $ids)) {
                                        echo '<img src="/static/style_default/image/ac_03.png">';
                                    }
                                ?>
                            </a>
                            <?php endforeach ?>
                    ...
                    <li class="roomsDescribe">
                        <em>房间描述</em>
                        <textarea name="room_description" id="describe" cols="30" rows="10"><?php echo $a_view_data['detail']['room_description']; ?></textarea>
                    </li>
                </ul>
                <input type="submit" value="确定修改" id="roomsSub"/>
            </form>

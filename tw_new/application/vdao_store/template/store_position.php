...
<body>
    ...
    <br><br>
    <button onclick="save_position()">保存设置</button>
    ...
    <script type="text/javascript">
    	$(function(){
    		var store_position_x = <?php echo $a_view_data[0]; ?>;
    		var store_position_y = <?php echo $a_view_data[1]; ?>;
			$("input[name='lngX']").val(store_position_x);
			$("input[name='latY']").val(store_position_y);
    		marker = new AMap.Marker({
				position: <?php echo '['.$a_view_data[0] . ',' . $a_view_data[1] . ']'; ?>,
				//title: '门店名称',
				map: mapObj
			});
    	});
        ...
        var clickEventListener=AMap.event.addListener(mapObj,'click',function(e){
            ...
            $('#lngX').css('color',getRandomColor());
            document.getElementById("latY").value=e.lnglat.getLat();
            $('#latY').css('color',getRandomColor());
        });
        ...
        function save_position() {
        	var position_x = $("input[name='lngX']").val();
        	var position_y = $("input[name='latY']").val();
        	$.ajax({
        		url: '<?php echo $this->router->url('position_update'); ?>',
        		type: 'POST',
        		dataType: 'json',
            ...
            if (tipArr&&tipArr.length>0) {
                for (var i = 0; i < tipArr.length; i++) {
                    resultStr += "<div id='divid" + (i + 1) + "' onmouseover='openMarkerTipById(" + (i + 1)
                                ...
                                + ",this)' style=\"font-size: 13px;cursor:pointer;padding:5px 5px 5px 5px;\"" + "data=" + tipArr[i].adcode + ">" + tipArr[i].name + tipArr[i].district + "</div>";
                }
            }
            else  {

...
class MessageRepository extends BaseRepository
{
    ...
    public function beforeGetList($query)
    {
        ...
        return $query->order_by(['message_post_at' => 'DESC']);
    }
    ...
    {
        $user_info = app('user_info');
        $this->db->update(get_table('message'), [
            'message_is_read' => 1
        ], ['message_notice_uid' => $user_info->user_id]);
        foreach ($rows as &$row) {
            if (($row['message_info_id'] != 0) && $message_info = $this->db->get_row(get_table('message_info'), ['id' => $row['message_info_id']], 'message_info')) {

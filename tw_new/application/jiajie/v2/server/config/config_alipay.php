<?php
$a_config_alipay = [
    // 支付宝分配给开发者的应用ID
    'app_id'      => '2016030501185352',
    // 支付宝接口请求地址
    'api_url'     => 'https://openapi.alipay.com/gateway.do',
    // 商户私钥
    'key_private' => "",
    // 支付宝公钥，公钥查看地址：https://openhome.alipay.com/platform/keyManage.htm
    'key_public'  => "",
    // 调用的接口版本，固定为：1.0
    'version'     => '1.0',
    // 请求使用的编码格式，如utf-8,gbk,gb2312等
    'charset'     => 'utf-8',
    // 仅支持JSON
    'format'      => 'json',
    // 商户生成签名字符串所使用的签名算法类型，目前支持RSA2和RSA，推荐使用RSA2，（必须大写，否则报错）
    'sign_type'   => 'RSA2',
    // 同步通知地址
//	'return_url' => 'http://wy120.7dugo.com/ret.html',
    // 异步通知地址
//	'notify_url' => 'http://jiajie-server.7dugo.com/order.pay.ret-alipay'

    'auth_config' => [
        'app_id'                => '2018110261994103',
        // 请填写开发者私钥去头去尾去回车，一行字符串
        'rsa_private_key'       => str_replace(['-----BEGIN RSA PRIVATE KEY-----', '-----END RSA PRIVATE KEY-----', PHP_EOL], '', file_get_contents(__DIR__ . '/pem/alipay_auth/rsa_private_key.pem')),
        // 请填写支付宝公钥，一行字符串
        'alipay_rsa_public_key' => '',
    ],
];

	...
	<div>
		<div class="storeApply">
								...
								<div class="idCardA">
									...
									<ul>
										<li v-if="detailGet.store_id_card_opposite">
											<img :src="$get_thumb_img(detailGet.store_id_card_opposite,150,110)" />
										</li>
									</ul>
								</div>
								<div class="idCardB">
									...
									<ul>
										<li v-if="detailGet.store_id_card_positive">
											<img :src="$get_thumb_img(detailGet.store_id_card_positive,150,110)" />
										</li>
									</ul>
								</div>
							</div>
								...
								<div class="qualA">
									...
									<ul>
										...
										<li v-if="detailGet.store_zizhi_opposite">
											<img :src="$get_thumb_img(detailGet.store_zizhi_opposite,150,110)" />
										</li>
									</ul>
								</div>
								<div class="qualB">
									...
									<ul>
										<li v-if="detailGet.store_zizhi_positive">
											<img :src="$get_thumb_img(detailGet.store_zizhi_positive,150,110)" />
										</li>
									</ul>
								</div>
							</div>
							...
							<div class="storeImage">
								...
								<ul>
									<li v-for="(item,index) in detailGet.store_pic">
										...
										<img :src="$get_thumb_img(item,150,110)" />
									</li>
								</ul>
							</div>
			...
			<div v-else>
				...
				<div class="btnBox" v-if="(storeSta  && !user_type) || user_type != 3">
				...
				<div class="infoForm">
					<div class="infoBox" v-show="infoShow">
						<van-cell-group>
							<van-field v-if="storeSta" v-model="storeSta" type="number" disabled label="店铺ID" placeholder="请输入所属店铺ID" />
							<van-field v-if="storeSta || user_type && user_type != 3" v-model="dragData.address" label="住址信息" placeholder="请选择住址" @click="Receadd" style="font-size: .1rem;" />
							...
							<div class="idCard_box">
								<div class="idCardA">
									...
									<ul>
										...
										<li v-show="path_imgA">
											<img :src="$get_thumb_img(path_imgA,150,110)" />
										</li>
									</ul>
								</div>
								<div class="idCardB">
									...
									<ul>
										<li v-show="path_imgB">
											<img :src="$get_thumb_img(path_imgB,150,110)" />
										</li>
									</ul>
								</div>
							</div>
							<div class="qual_box">
								...
								<div class="qualA">
									...
									<ul>
										...
										<li v-show="qual_imgA">
											<img :src="$get_thumb_img(qual_imgA,150,110)" />
										</li>
									</ul>
								</div>
								<div class="qualB">
									...
									<ul>
										<li v-show="qual_imgB">
											<img :src="$get_thumb_img(qual_imgB,150,110)" />
										</li>
									</ul>
								</div>
							</div>
	...
	export default {
		...
		methods: { //方法
			...
			info_sub() {
				...
				if(that.storeSta) {
//					if(that.storeSta != "" && that.username != "" && that.storecontact != "" && that.storeIdcard != "" && (that.path_imgA != "" && that.path_imgB != "")) {
...
//							that.$toast('请选择住址');
							...
							lists.store_user_lat = 22.97721; //定位坐标
							lists.store_user_lng = 113.31074;

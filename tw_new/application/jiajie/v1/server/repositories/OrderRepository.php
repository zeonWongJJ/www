...
//            ->where(['a.id' => $id])
//            ->join(['jiajie_order_entity' => 'c'], ['c.order_sn' => 'a.order_sn'], 'inner')
//            ->select($field);
//        return $query;
//    }
...
//            ->join(['jiajie_order_entity' => 'c'], ['c.order_sn' => 'a.order_sn'], 'inner')
    ...
    {
        foreach ($rows as &$row) {
            $row['add_time']            = date('Y-m-d H:s:i', $row['add_time']);
            $row['order_amount']        = number_format($row['order_amount'] / 100, 2, '.', ',');
            $row['order_actual_amount'] = number_format($row['order_actual_amount'] / 100, 2, '.', ',');
        }

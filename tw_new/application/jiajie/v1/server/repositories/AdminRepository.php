...
//    }
    ...
    {
        if (!$this->db->get_total(get_table('role'), ['id' => $insert['role_id']])) {
            throw new \RuntimeException('角色组不存在');
        }
...
//    }
    ...
    {
        if (!$this->db->get_total('jiajie_role', ['id' => $update['user_role']])) {
            throw new \RuntimeException('角色组不存在');
        }
    ...
    {
        $this->db->delete('jiajie_access_token', [
            'user_id'     => $id
            , 'user_type' => 'admin'
        ]);
    }

...
class CommentRepository extends BaseRepository
{
    ...
    {
        $_query = $query->join(['user' => 'b'], ['b.user_id' => 'a.user_id'], 'INNER')->select('b.user_pic, b.user_name, a.*', false);
        return $_query;
    }
    ...
    public function beforeGetList($query)
    {
        $_query = $query->join(['user' => 'b'], ['a.user_id' => 'b.user_id'], 'INNER')
            ...
            ->select('b.user_nickname as user_name, b.user_pic, a.*, c.service_name', false);
    ...
    public function afterInsertHook(array $insert)
    {
        ...
        $this->db->update(get_table('order'), $order_update, ['order_sn' => $insert['comment_order_sn']]);
        ...
        if (3 != $order_info['order_type']) {
            ...
            if (1 == $order_info['order_type']) { // 只在支付的是服务时计算好评+等级
                ...
                if (1 == $insert['comment_type_star']) { // 好评
                    ...
                    $evaluate_update['store_zp_count']     = $store_info['store_zp_count'] + 1;
                    $service_pj_update['service_zp_count'] = $service_info['service_zp_count'] + 1;
                } else { // 差评
                    $evaluate_update['store_cp_count']     = $store_info['store_cp_count'] + 1;
                ...
                $service_update['service_average_score'] = sprintf('%.2f', $service_update['service_score_count'] / $service_update['service_comment_count']); // 计算平均得分
                $this->db->update(get_table('service'), $service_update, ['id' => $service_info['id']]);
            }

            $order_model->orderSettlement($insert['comment_order_sn']); // 结算订单
            $order_model->reward($insert['comment_order_sn']); // 上级获得回佣

            // 返还积分
            /** @var JifenModel $jifen_model */
            $jifen_model = Factory::getFactory('jifen');
            $jifen_model->restore($order_info['id']); // 执行积分返还
            ...
            $this->db->update(get_table('order'), ['order_bis_state_dsc' => 'COMPLETED'], ['order_sn' => $insert['comment_order_sn']]);
        } else {

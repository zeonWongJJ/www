          ...
          <li class="list_coms">
            <div class="com_com">
              <div>
                ...
                <img :src="getServiceImg(lists)" v-else/>
              </div>
              <div class="com_com_x">
...
<script>
  ...
  export default {
    ...
    methods: { //方法
      getServiceImg (lists) {
        if (lists.service_img) {
          return this.uploadFileUrl + lists.service_img[0]
        }
        return require('../../assets/img/logo_h.png');
      },
      ...
      init() {
        ...
        that.$fetch('user_info_get',{}).then(rs =>{
          this.cons_jifen = parseFloat(rs.data.user_score)
          this.user_balance = parseFloat(rs.data.user_balance)
        })
      },
      //显示选择地址
      addsadd() {
...
//				      return
        ...
        if (this.onshows) {
          ...
          this.$fetch('user_buy_service', list, this.lists.id).then(rs =>{
          	if (!rs.error) {
              let order_sign = rs.data.order_sign; // 订单签名
              let order_sn = rs.data.order_sn; // 订单流水号
              window.location.href = `http://jiajie-server.7dugo.com/order.pay?order_sign=${order_sign}&order_sn=${order_sn}`
           } else {
              this.$toast(JSON.stringify(rs.msg));
            }
          })
      		this.onshows = true
        }

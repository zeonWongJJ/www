			...
			<div class="com_box">
				...
				<ul>
					<li class="list_coms">
						<div class="com_com">
							<div>
								<img v-if="list_com.subject_img.length == 0" :src="defaultServiceImages" />
			...
			init() {
				...
				if(that.$route.query.order_sn) {
					that.$fetch('order_get', list, that.$route.query.order_sn).then(rs =>{
						if(rs.error == 0) {
							that.addslists = rs.data.order_info
							that.list_com = rs.data.entity_row
						} else {
							that.$toast(rs.msg[0]);
						}
					})

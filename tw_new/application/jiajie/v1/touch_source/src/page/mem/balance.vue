	...
	export default{
		...
		methods:{
	    	...
	    	init(){//初始化请求
				...
				.then(rs =>{
					if(rs.error == 0){
						that.value = rs.data.user_balance;
					}else{
						that.$toast(rs.msg[0]);
					}
				})
				that.$fetch('user_balance_list', lists)
				.then(rs =>{
					if(rs.error == 0){
						that.page++//请求页数自加
						that.balance = rs.data;//覆盖本地数据
						if(rs.data.length != 10){//如果数据长度小于10证明下次请求没有数据
	    			...
	    			if(that.end){//如果end == true代表已无数据
						...
						.then(rs =>{
							if(rs.error == 0){
								setTimeout(() => {
									that.page++//请求页数自加
									that.balance = that.balance.concat(rs.data);//合并至本地数据
									if(rs.data.length != 10){//如果数据长度小于10证明下次请求没有数据
										setTimeout(() => {
								            done(true)//true返回已无数据
								        })

...
<script>
	...
	export default {
		...
		activated(){
			if(this.$store.state.path.from === '/release_dem_category' || this.$store.state.path.from === '/release_demand'){
				this.form.test = '';
				this.form.textareas = '';
				this.file_img = [];
				this.price = '';
		...
		methods: { //方法
			...
			pay_meng() {
				...
				if(that.form.test == '') {
					...
					that.$toast('描述不能为空');
				} else if(utils.returnFloat(that.price) <= 0) {
					console.log(utils.returnFloat(that.price))
					that.$toast('酬金太少咯');
					...
					lists.price = utils.returnFloat(that.price)
...
//				})
			...
			onRead(file) {
				...
				that.$fetch('uploadBase', lists).then(rs =>{
					if(rs.error == 0) {
						that.file_img.push(rs.data.path)
						that.$toast('上传成功');
					} else {
						that.$toast(rs.msg[0]);
					}

...
layui.config({
...
}).define(["fetch", "element", "jquery"], function (exports) {
    ...
    Tab.prototype.render = function () {
        var nav_bar = sessionStorage.getItem('nav_bar_str');
        if (!nav_bar) {
            var url = this.tabConfig.url;
            $.ajax(config.)
            console.log(url)
            fetch.ajax(url, {}, function (data) {
                if ($(".navBar").html() == '') {
                    nav_bar = navBar(data);
                    sessionStorage.setItem('nav_bar_str', nav_bar);

                    $(".navBar").html(nav_bar).height($(window).height() - 245);

...
class Bill_model extends TW_Model{
	...
	public function get_user_seat($user_id, $store_arr) {
		...
		$s_field = 'appointment_id,store_id,office_seatname';
		$a_order = [
			'appointment_id' => 'desc'
		];
		$a_data['appointment_price'] = $this->db->select("SUM(appointment_price) as appointment_price",false)->where_in('store_id', $store_arr)
						   ->get('appointment', $a_where,'', 0, 9999999999);
			$a_data['appointment'] = $this->db->where_in('store_id', $store_arr)
						   ->get('appointment', $a_where, $s_field, $a_order, 0, 9999999999);
		return $a_data;
	}
	//返回订座的总价
	public function get_seat_price($user_id, $store_arr) {
		...
		$s_field = 'actual_pay,appointment_id';
		
		$a_data = $this->db->where_in('store_id', $store_arr)
						   ->get('appointment', $a_where, $s_field, 0, 99999999);
		$appointment_price = 0.00;
		$id_str = '';
		if(!empty($a_data) && is_array($a_data)){
			foreach ($a_data as $val) {
				$appointment_price += $val['actual_pay'];
				$str .=$val['appointment_id'].',';
			}
		}
		$arr['appointment_id_str'] = rtrim($str ,',');
		$arr['actual_pay'] = $appointment_price;
		return $arr;
	}
	...
	public function get_store_info($store_arr) {
		...
		if($store_arr){
				$a_data = $this->db->where_in('store_id', $store_arr)
						   ->get('store', "", $s_field, $a_order, 0, 9999999);	
		}else{
				$a_data = $this->db->get('store', "", $s_field, $a_order, 0, 9999999);					
		}

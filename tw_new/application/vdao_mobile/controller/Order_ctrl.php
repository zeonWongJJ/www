	...
	public function goods_order() {
		...
		if ( ! empty($i_order)) {
			if ($i_order == 55) {
				...
				$a_where .= ($a_where ? ' AND ' : '') . "`order_state` >= 20 AND `order_state` <= 25";
			} else {
		...
		$a_data['product'] = $this->db->from('order as a')
         ...
         foreach($a_data['product'] as $key => $val ) {
         	$val['needpay'] = $val['order_price'] - $val['shipping_fee'] - $val['seat_deduction'] -$val['use_jife'];
         	if($val['needpay'] <0) $val['needpay'] = 0.00;
         	$a_data['product'][$key] = $val;
         }
...
/********************************* 产品订单状态的反应 ****************************/
	...
	public function order_confirm() {
		...
		if( ! empty($i_fukuan)) {
			...
			if ((time() - $a_tert['order_time']) > 600) {
		    	...
		    	if ($a_tert['payment_code'] == 'online') {
					...
					$this->db->insert('test', ['test_content' => json_encode($a_result),'order_id' => $a_tert['order_id']]);
					if ($a_result['return_code'] == 'SUCCESS' && $a_result['result_code'] == 'SUCCESS') {
						...
						echo json_encode(array('code' => 55, 'usname' => $a_result['err_code_des']));
						die;
					}
		...
		if ( ! empty($i_ensure)) {
            ...
            $this->db->insert('order_log', $a_name);
			$a_ensure = $this->db->update('order', ['order_state' => 10, 'time_finnshed' => $_SERVER['REQUEST_TIME']], ['order_id' => $i_ensure, 'user_id' => $_SESSION['user_id']]);
			if ($a_ensure) {

				//完成的订单统计到表里
				$a_wheres =['order_id' => $i_ensure]; 
				$s_field = "order_id, store_id, order_price, score_tostore, order_type ,time_create,order_count";
				$order_statistic = $this->db->get_row("order",$a_wheres , $s_field);
				$order_month = date("Ym", $order_statistic['time_create']);

				if($order_statistic['order_type'] ==2) {
					$this->order_model->update_accounttbl($order_statistic['store_id'] , $order_statistic['order_price'] , 
						$order_statistic['score_tostore'],$order_statistic['order_count'], $order_month);
				} else {
				$this->order_model->update_accounttbl($order_statistic['store_id'] , $order_statistic['order_price'] , 0 ,$order_statistic['order_count'],$order_month);					
				}
...
/********************************************************************************/
 ...
 function refund_test() {
 		$a_where = [];
 		$a_order = [
		  'test_id' => 'desc'
		];
 		$a_result = $this->db->get('test', '', $s_field='', $a_order);

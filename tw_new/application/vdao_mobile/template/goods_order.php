...
<html>
    ...
    <!-- 拉框结束 -->
		<div class="main">
			<header class="head"><a href="nuser_center"><img src="static/style_default/images/gouxiang_18.png"/></a><i>餐饮订单</i></header>
					...
					<li>
						...
						<div class="control clearfix">
							<?php if ($product['order_state'] <= 2) {
								...
								echo '<a href="javascript:;" class="goPay" data-needpay ='.$product['needpay'].' value='.$product['order_id'].'>去付款</a>
							<a href="javascript:;" class="cancel weifuk" value='.$product['order_id'].'>取消订单</a>';
							} else if ($product['order_state'] == 20) {
								if(empty($product['store_contact'])) {

								} else {
									echo '<a href="tel:'.$product['store_contact'].'" class="contactBus">联系商家</a>';
								}								
								if ($product['order_time'] < strtotime("-5 minute")) {

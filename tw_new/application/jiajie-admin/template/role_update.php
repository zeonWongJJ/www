...
<form class="layui-form">
  ...
  <div class="layui-form-item">
    ...
    <div class="layui-input-inline">
      <input type="text" name="role_name" required lay-verify="required" placeholder="角色名称" autocomplete="off" class="layui-input" value="<?php echo $a_view_data['role']['role_name']; ?>">
    </div>
  ...
  <div class="layui-form-item">
    ...
    <div class="layui-input-inline">
      <select name="role_state" lay-verify="required">
        <option value=""></option>
        <option value="1" <?php if ($a_view_data['role']['role_state'] == 1) { echo 'selected'; } ?>>开启</option>
        <option value="2" <?php if ($a_view_data['role']['role_state'] == 2) { echo 'selected'; } ?>>关闭</option>
      </select>
    </div>
  </div>
  <div class="layui-form-item">
    ...
    <div class="layui-input-block">
		<table class="layui-table">
		  <tbody>
		  	<?php if (!empty($a_view_data['role']['auth_ids'])) {
		  		$auth_ids = explode(',', $a_view_data['role']['auth_ids']);
		  	} else {
		  		$auth_ids = array();
		  	} ?>
		    ...
		    <tr id="tr_<?php echo $value['auth_id']; ?>">
		      <td>
		      	<input type="checkbox" name="role_auth[]" title="<?php echo $value['auth_name']; ?>" lay-skin="primary" value="<?php echo $value['auth_id']; ?>" lay-filter="p_auth_choose" <?php if (in_array($value['auth_id'], $auth_ids)) { echo 'checked'; } ?>>
		      </td>
		      <td class="td_two">
			  <?php foreach ($a_view_data['auth'] as $k => $v): ?>
		      <?php if ($v['auth_pid'] == $value['auth_id']) { ?>
		      	<input type="checkbox" name="role_auth[]" title="<?php echo $v['auth_name']; ?>" lay-skin="primary" value="<?php echo $v['auth_id']; ?>" <?php if (in_array($v['auth_id'], $auth_ids)) { echo 'checked'; } ?>>
		      <?php } ?>
			  <?php endforeach ?>
		      </td>
		    </tr>
  ...
  <div class="layui-form-item layui-form-text">
    ...
    <div class="layui-input-block">
      <textarea name="role_description" placeholder="请输入内容" class="layui-textarea"><?php echo $a_view_data['role']['role_description']; ?></textarea>
    </div>
  </div>
  <div class="layui-form-item">
    <div class="layui-input-block">

...
if($verify_result) {//验证成功
    ...
    if($_POST['trade_status'] == 'TRADE_FINISHED') {
		...
		//更新订单表
		$b_order = $this->db->where_in('pay_sn', $a_pay_sn)->update('order', ['order_state' => '20', 'time_payment' => $_SERVER['REQUEST_TIME']]);
		
		//更新订单支付表
		$b_order_pay = $this->db->where_in('pay_sn', $pay_sn)->update('order_pay', ['api_pay_state' => '1']);

		$a_data = $this->db->where_in('pay_sn', $pay_sn)->get('order',['buyer_id' => $_SESSION['user_id']],'order_id');

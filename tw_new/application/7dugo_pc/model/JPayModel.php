...
/**
 * @property \utils\Medoo database
 */
class JPayModel extends BaseModel
{
    const ALIPAY = 'alipay';
    const WECHAT = 'wechat';
    ...
    public function pay(array $order)
    {
        ...
        if ($this->pay == 'wechat') { // 微信支付
            $data = [
                ...
                'notify_url'       => 'http://www.7dugo.com/order.pay.return-wechat',
...
//            $mweb_url = $this->pay_way->wap($data);
            if (is_weixin()) {
                ...
                $data['url_success'] = 'http://www.7dugo.com/order.pay.return-wechat?out_trade_no=' . $order['out_trade_no'];
...
//                header('location:' . $result );
                // . '&redirect_uri=' . urlencode('http://www.7dugo.com/order.pay.return-wechat&out_trade_no=' . $order['out_trade_no'])
            } else {
                app('load')->library('wxpay_h5', '', [$data]);
                $result = $this->wxpay_h5->pay();
                header('location:' . $result['mweb_url'] . '&redirect_url=' . urlencode('http://www.7dugo.com/order.pay.return-wechat?out_trade_no=' . $order['out_trade_no']));
            }
//            echo <<<EOF
...
//            header("location:$url");
            ...
            $a_param = [
                ...
                'url_front' => 'http://www.7dugo.com/order.pay.return-bankcard?out_trade_no=' . $order['out_trade_no'],
                'url_back'  => 'http://www.7dugo.com/order.pay.return--bankcard?out_trade_no=' . $order['out_trade_no'],
            ];
    ...
    public function transfer(array $transfer)
    {
        ...
        if ($this->pay == 'alipay') {
            ...
            $this->database->insert('withdraw_logs', [

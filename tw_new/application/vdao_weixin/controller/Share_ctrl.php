...
class Share_ctrl extends TW_Controller
{
    ...
    public function share_ordermore()
    {
        ...
        if (!empty($a_data)) {
            ...
            echo json_encode(['code' => 200, 'msg' => '获取成功', 'data' => $new_data]);
        } else {
            echo json_encode(['code' => 400, 'msg' => '没有更多数据啦', 'data' => '']);
        }
    }
    ...
    public function share_ordercancel()
    {
        ...
        if ($isrefund == true) {
            ...
            if ($i_result) {
                ...
                echo json_encode(['code' => 200, 'msg' => '取消订单成功']);
            } else {
    ...
    public function share_orderacct()
    {
        ...
        if ($i_result) {
            ...
            echo json_encode(['code' => 200, 'msg' => '接单成功']);
        } else {
                ...
                */
                ...
                if ($a_this_user['user_referee']) {
                    ...
                    if ($a_referee_row['is_shopman'] == 1 && $a_referee_row['shopman_state'] == 1) {
                        $order_commission = round(($shopman_referee_ratio / 100) * $a_this_order['goods_amount']);
                    } else {
                        $order_commission = round(($user_consume_ratio / 100) * $a_this_order['goods_amount']);
                    }
                    $a_order_where  = [
                        'order_id' => $order_id,
                    ];
                ...
                if ($a_this_user['is_shopman'] == 1 && $a_this_user['shopman_state'] == 1) {
                    $add_user_score = round(($shopman_score_retio / 100) * $a_this_order['goods_amount']);
                } else {
                    $add_user_score = round(($user_score_ratio / 100) * $a_this_order['goods_amount']);
                }
                // 更新自己的积分及消费金额
                $a_where_thisuser = [
    ...
    public function get_goods_list()
    {
        ...
        if (empty($a_data['goods'])) {
            echo json_encode(['code' => 400, 'msg' => '没有更多数据了', 'data' => '']);
        } else {
            foreach ($a_data['goods'] as $key => $value) {
                $value['apply_time'] = date('Y.m.d', $value['apply_time']);
                $goods[]             = $value;
            }
            echo json_encode(['code' => 200, 'msg' => '获取成功', 'data' => $goods]);
        }
    }
    ...
    public function share_goods_del()
    {
        ...
        if (empty($a_dele)) {
            ...
            echo json_encode(['code' => 200, 'msg' => '删除成功', 'data' => $i_id]);
        }
    }
    ...
    public function share_goods_ster()
    {
        ...
        if (empty($a_dele)) {
            ...
            echo json_encode(['code' => 200, 'msg' => '修改成功', 'data' => $i_id]);
        }
    }
}

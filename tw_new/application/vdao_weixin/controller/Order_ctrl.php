...
/********************************* 产品订单状态的反应 ****************************/
	...
	public function order_confirm() {
		...
		if(!empty($i_weifuk)) {
			...
			$a_weifuk = $this->db->update('order', ['order_state' => 1, 'time_finnshed' => $_SERVER['REQUEST_TIME']], ['order_id' => $i_weifuk, 'user_id' => $_SESSION['user_id']]);
			if ($a_weifuk) {
		...
		if( ! empty($i_fukuan)) {
			...
			if ((time() - $a_tert['order_time']) > 600) {
		    	...
		    	if ($a_tert['payment_code'] == 'online') {
		            ...
		            $this->db->insert('order_log', $a_name);
					$a_weifuk = $this->db->update('order', ['order_state' => 1, 'time_finnshed' => $_SERVER['REQUEST_TIME']], ['order_id' => $i_fukuan, 'user_id' => $_SESSION['user_id']]);
					if ($a_weifuk) {
					 	echo json_encode(array('code'=>50, 'usname' => '订单取消成功'));
					 	die;
					} else {
					...
					if ($zhihu['code'] == 10000) {
						...
						$this->db->insert('order_tracking', $a_track);
						// 把订单状态改为取消
	    				$this->db->update('order', ['order_state' => 1, 'time_finnshed' => $_SERVER['REQUEST_TIME']], ['order_id' => $i_fukuan, 'user_id' => $_SESSION['user_id']]);
	    				if( ! empty($a_tert['use_jife'])){
		    				// 增加会员积分表
		    				$a_jife = [
					...
					if ($a_result['return_code'] == 'SUCCESS') {
						...
						$this->db->insert('order_tracking', $a_track);
						// 把订单状态改为取消
	    				$this->db->update('order', ['order_state' => 1, 'time_finnshed' => $_SERVER['REQUEST_TIME']], ['order_id' => $i_fukuan]);
		    			//增加订单处理历史表
						$a_log = [
							'order_id' => $i_fukuan,
							'log_msg'  => $_SESSION['user_name'].'付款订单取消',
						...
						if ($a_result['origRespCode'] == '00') {
							...
							if ($this->unionpay_geteway->verify($a_result)) {
								if ($a_result['respCode'] == '00') {
									$this->db->insert('order_tracking', $a_track);
									// 把订单状态改为取消
				    				$this->db->update('order', ['order_state' => 1], ['order_id' => $id]);
				    				//把返回的金额和积分返回给用户
